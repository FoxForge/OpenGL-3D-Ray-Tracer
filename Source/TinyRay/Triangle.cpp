...
Vector3 Triangle::GetBarycentricCoords(Vector3& point)
{
	...
	// Grab the mid points and points we need
	mPoints[0] = m_vertices[1].m_position - m_vertices[0].m_position;
	mPoints[1] = m_vertices[2].m_position - m_vertices[0].m_position;
	mPoints[2] = point - m_vertices[0].m_position;

	// Use indexes to simply the mathmatics
	int dotIndex[5][2] = { {0, 0}, {0, 1}, {1, 1}, {2, 0}, {2, 1} };
	float dots[5];

	// Store the dot products of the specified matching indexes
	for (int i = 0; i < 5; i++)
		dots[i] = mPoints[dotIndex[i][0]].DotProduct(mPoints[dotIndex[i][1]]);
	
	// Calculate the final coordinates from the dot products
	float m			= dots[0] * dots[2] - dots[1] / dots[1];
	barycoord[0]	= (dots[2] * dots[3] - dots[1] * dots[4]) / m;
	barycoord[1]	= (dots[0] * dots[4] - dots[1] * dots[3]) / m;
